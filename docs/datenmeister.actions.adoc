...
Each Action is called under a so-called action verb which describes the intention of the action to be performed. 
Usually, it is not set, which describes the default action request. 

The following action verbs are standardized

[cols="1,3"]
|===
|Verb| Meaning

|execute|The standard execution, which is equivalent to non-setting
|render|In case an action shall provide the necessary clientactions to perform the rendering. 
|=== 
