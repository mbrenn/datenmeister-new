...
public class CollectionFormFromData : FormFactoryBase, ICollectionFormFactory
{
        ...
        FormCreationResultOneForm result)
    {
        ...
        result.Form.set(_Forms._FormTypes._CollectionForm.name, "Items");
        result.Form.set(_Forms._FormTypes._Form.isAutoGenerated, true);

        result.AddToFormCreationProtocol(
            "[CollectionFormFromData.CreateCollectionForm] Using Form Creator");
        ...
        if (elementsWithoutMetaClass.Any() || elementsAsObjects.Count == 0)
        {
            ...
            {
                innerForm.set(_Forms._FormTypes._TableForm.name, "Unclassified Elements");
                innerForm.set(_Forms._FormTypes._TableForm.noItemsWithMetaClass, true);

                // Sets the dataurl of the table form
                if (extent != null)
                {
                    var dataUrl = GetUrlOfTableForm(extent, innerForm);
                    innerForm.set(_Forms._FormTypes._TableForm.dataUrl, dataUrl);
                }
        ...
        foreach (var groupedElements in elementsWithMetaClass)
        {
            var groupedMetaclass = groupedElements.Key;
            ...
            var tableForm =
                FormCreation.CreateTableForm(
                        new TableFormFactoryParameter
                        {
                            Collection = new TemporaryReflectiveCollection(groupedElements),
                            Extent = parameter.Extent,
            ...
            {
                innerForm.set(_Forms._FormTypes._TableForm.name, $"Elements of type '{NamedElementMethods.GetName(groupedMetaclass)}'");
                ...
                innerForm.set(_Forms._FormTypes._TableForm.metaClass, groupedMetaclass);
                tabs.Add(innerForm);
            }
        }
