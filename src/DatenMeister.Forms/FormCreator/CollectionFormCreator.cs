...
public class CollectionFormCreator : FormCreator, ICollectionFormFactory
{
        ...
        FormFactoryContext creationMode)
    {
        ...
        form.set(_Forms._CollectionForm.name, $"Collection Form by '{extent}'");
        return form;
    }
    ...
    /// <param name="context">The creation mode defining whether metaclass are used or not</param>
        ...
        CollectionFormConfiguration? extentFormConfiguration)
    {
        ...
        var result = GetMofFactory(context).create(_Forms.TheOne.__CollectionForm);
        ...
        if (elementsWithoutMetaClass.Any() || elementsAsObjects.Count == 0)
        {
            ...
            var form = tableFormCreator.CreateTableFormForCollection(
                ...
                context with { IsForTableForm = true, AllowFormModifications = false });
            if (form == null)
            {
    ...
    public IElement CreateCollectionFormForMetaClass(IElement metaClass,
        FormFactoryContext? creationMode = null)
    {
        ...
        collectionForm.set(_Forms._ObjectForm.name, $"ListForm for '{NamedElementMethods.GetName(metaClass)}'");
