...
public class ItemFormatter
{
    ...
    {
        var tabs = extentForm.getOrDefault<IReflectiveCollection>(_Forms._FormTypes._CollectionForm.tab);
        if (tabs == null)
        {
    ...
    private void FormatCollectionByTab(IEnumerable<object?> collection, IObject listForm)
    {
        ...
        {
            var fieldName = field.getOrDefault<string>(_Forms._FieldTypes._FieldData.title);

            fieldName = string.IsNullOrEmpty(fieldName)
                ? field.getOrDefault<string>(_Forms._FieldTypes._FieldData.name)
                : fieldName;
            if (fieldName == null)
            {
                continue;
        ...
        foreach (var item in collection.OfType<IObject>())
        {
            ...
            {
                var fieldName = field.getOrDefault<string>(_Forms._FieldTypes._FieldData.name);
                if (fieldName == null)
                {
                    continue;
                }
                ...
                HtmlElement value;
                if (field.metaclass?.equals(_Forms.TheOne.FieldTypes.__MetaClassElementFieldData) == true)
                {
                ...
                else if (field.metaclass?.equals(_Forms.TheOne.FieldTypes.__FullNameFieldData) == true)
                {
                ...
                {
                    if (field.getOrDefault<bool>(_Forms._FieldTypes._FieldData.isEnumeration))
                    {
                    ...
                    {
                        var contentValue = item.getOrDefault<string>(fieldName);
                        value = string.IsNullOrEmpty(contentValue) 
                            ? new HtmlRawString("<i>unset</i>")
                            : contentValue;
                    }
                }
    ...
    public void FormatItem(IObject item, IObject detailForm)
    {
        ...
        var tabs = detailForm.getOrDefault<IReflectiveCollection>(_Forms._FormTypes._CollectionForm.tab);
        if (tabs != null)
        {
    ...
    private static void CreateDetailRowsForFields(IObject item, IObject form, HtmlTable table)
    {
        ...
        {
            var fieldName = field.getOrDefault<string>(_Forms._FieldTypes._FieldData.name);
            var title = field.getOrDefault<string>(_Forms._FieldTypes._FieldData.title);
            ...
            {
                var realValue = item.getOrDefault<string>(fieldName);
