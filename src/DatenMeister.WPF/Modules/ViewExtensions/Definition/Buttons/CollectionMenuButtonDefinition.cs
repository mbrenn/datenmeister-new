...
/// </summary>
public class CollectionMenuButtonDefinition(
    string name,
    Action<IReflectiveCollection> onPressed,
    string? imageName,
    string categoryName,
    int priority = 0)
    : NavigationButtonDefinition(name, NavigationScope.Collection, imageName, categoryName, priority)
{
    ///<summary>
    /// Gets the action being executed when the user clicked upon the button
    /// </summary>
    public Action<IReflectiveCollection> OnPressed { get; } = onPressed;
}