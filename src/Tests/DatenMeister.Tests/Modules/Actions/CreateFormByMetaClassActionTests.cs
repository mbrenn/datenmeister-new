...
public class CreateFormByMetaClassActionTests
{
    ...
    public async Task TestCreationOfObjectForm()
    {
        ...
        var action = InMemoryObject.CreateEmpty(_Actions.TheOne.__CreateFormByMetaClass);
        action.set(
            _Actions._CreateFormByMetaClass.creationMode,
            CreateFormByMetaclassCreationMode.Object);
        action.set(_Actions._CreateFormByMetaClass.metaClass, _zipModel.ZipCode);
        ...
        Assert.That(form.metaclass!.equals(_Forms.TheOne.__ObjectForm), Is.True);

        var detailForms = form.getOrDefault<IReflectiveSequence>(_Forms._CollectionForm.tab);
    ...
    public async Task TestCreationOfCollectionForm()
    {
        ...
        Assert.That(form, Is.Not.Null);
        Assert.That(form!.metaclass, Is.Not.Null);
        Assert.That(form.metaclass!.equals(_Forms.TheOne.__CollectionForm), Is.True);
    ...
    public async Task TestCreationOfObjectAssociationForm()
    {
        ...
        var formCreationFactory = new NewFormCreationContextFactory(dm.WorkspaceLogic, dm.ScopeStorage);
        var context = formCreationFactory.Create();
        
        var autoGeneratedForm = 
            FormCreation.CreateObjectFormForMetaClass(_zipModel.ZipCodeWithState!, context).Form;
        Assert.That(autoGeneratedForm, Is.Not.Null);
        
        Assert.That(autoGeneratedForm.getOrDefault<bool>(_Forms._ObjectForm.isAutoGenerated), Is.True);
        ...
        action.set(_Actions._CreateFormByMetaClass.metaClass, _zipModel.ZipCodeWithState);
        ...
        context = formCreationFactory.Create();
        var objectForm = FormCreation.CreateObjectFormForMetaClass(_zipModel.ZipCodeWithState, context).Form;
        Assert.That(objectForm, Is.Not.Null);
        
        var first = 
            newElements
                .OfType<IElement>()
                .Single(x => x.metaclass?.equals(_Forms.TheOne.__ObjectForm) == true);
        var countDetailForm = objectForm.getOrDefault<IReflectiveSequence>(_Forms._ObjectForm.tab)
            .Count();
        var countNewForm = first.getOrDefault<IReflectiveSequence>(_Forms._ObjectForm.tab).Count();
        Assert.That(countDetailForm, Is.EqualTo(countNewForm));
        Assert.That(first.metaclass?.equals(_Forms.TheOne.__ObjectForm), Is.True);
        Assert.That(
            first.getOrDefault<IReflectiveSequence>(_Forms._ObjectForm.tab)
                .OfType<IElement>().Single()
                .metaclass?
                .equals(_Forms.TheOne.__RowForm),
            Is.True);

        var rowForm = first.getOrDefault<IReflectiveSequence>(_Forms._ObjectForm.tab)
            .OfType<IElement>().Single();
        Assert.That(rowForm.getOrDefault<IReflectiveSequence>(_Forms._RowForm.field)
