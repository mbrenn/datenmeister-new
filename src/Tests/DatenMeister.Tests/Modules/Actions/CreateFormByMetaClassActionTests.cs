...
public class CreateFormByMetaClassActionTests
{
    ...
    public async Task TestCreationOfObjectForm()
    {
        ...
        var action = InMemoryObject.CreateEmpty(_Actions.TheOne.Forms.__CreateFormByMetaClass);
        action.set(
            _Actions._Forms._CreateFormByMetaClass.creationMode,
            CreateFormByMetaclassCreationMode.Object);
        action.set(_Actions._Forms._CreateFormByMetaClass.metaClass, _zipModel.ZipCode);
        ...
        Assert.That(form.metaclass!.equals(_Forms.TheOne.FormTypes.__ObjectForm), Is.True);

        var detailForms = form.getOrDefault<IReflectiveSequence>(_Forms._FormTypes._CollectionForm.tab);
        ...
        Assert.That(detailForm!.metaclass!.equals(_Forms.TheOne.FormTypes.__RowForm));
    }
    ...
    public async Task TestCreationOfCollectionForm()
    {
        ...
        Assert.That(form, Is.Not.Null);
        Assert.That(form!.metaclass, Is.Not.Null);
        Assert.That(form.metaclass!.equals(_Forms.TheOne.FormTypes.__CollectionForm), Is.True);
    ...
    public async Task TestCreationOfObjectAssociationForm()
    {
        ...
        var formCreationFactory = new FormCreationContextFactory(dm.WorkspaceLogic, dm.ScopeStorage);
        ...
        var autoGeneratedForm =
            FormCreation.CreateObjectForm(
                ...
                {
                    MetaClass = _zipModel.ZipCodeWithState!
                }, context).Form;
        Assert.That(autoGeneratedForm, Is.Not.Null);
        
        Assert.That(autoGeneratedForm.getOrDefault<bool>(_Forms._FormTypes._ObjectForm.isAutoGenerated), Is.True);
        ...
        action.set(_Actions._Forms._CreateFormByMetaClass.metaClass, _zipModel.ZipCodeWithState);
        ...
        context = formCreationFactory.Create(string.Empty);
        var objectForm =
            FormCreation.CreateObjectForm(
                new ObjectFormFactoryParameter
                {
        ...
        var first = 
            newElements
                .OfType<IElement>()
                .Single(x => x.metaclass?.equals(_Forms.TheOne.FormTypes.__ObjectForm) == true);
        var countDetailForm = objectForm.getOrDefault<IReflectiveSequence>(_Forms._FormTypes._ObjectForm.tab)
            .Count();
        var countNewForm = first.getOrDefault<IReflectiveSequence>(_Forms._FormTypes._ObjectForm.tab).Count();
        Assert.That(countDetailForm, Is.EqualTo(countNewForm));
        Assert.That(first.metaclass?.equals(_Forms.TheOne.FormTypes.__ObjectForm), Is.True);
        Assert.That(
            first.getOrDefault<IReflectiveSequence>(_Forms._FormTypes._ObjectForm.tab)
                .OfType<IElement>().Single()
                .metaclass?
                .equals(_Forms.TheOne.FormTypes.__RowForm),
            Is.True);

        var rowForm = first.getOrDefault<IReflectiveSequence>(_Forms._FormTypes._ObjectForm.tab)
            .OfType<IElement>().Single();
        Assert.That(rowForm.getOrDefault<IReflectiveSequence>(_Forms._FormTypes._RowForm.field)
