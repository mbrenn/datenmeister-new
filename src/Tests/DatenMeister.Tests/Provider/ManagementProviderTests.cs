...
public class ManagementProviderTests
{
    ...
    public async Task TestSettingOfUri()
    {
        ...
        userExtent!.set(_Management._Extent.uri, "dm:///newusers");

        Assert.That(userExtent.get(_Management._Extent.uri), Is.EqualTo("dm:///newusers"));
    ...
    public async Task TestRemovalOfExtentViaProvider()
    {
        ...
        Assert.That(foundExtent.get<string>(_Management._Extent.uri), Is.EqualTo("dm:///test"));
        ...
        managementExtent)> GetInitializedWorkspace()
    {
        ...
        var loadConfig = InMemoryObject.CreateEmpty(
            _ExtentLoaderConfigs.TheOne.__InMemoryLoaderConfig);
        loadConfig.set(_ExtentLoaderConfigs._InMemoryLoaderConfig.name, "dm:///test");
        loadConfig.set(_ExtentLoaderConfigs._InMemoryLoaderConfig.extentUri, "dm:///test");
        loadConfig.set(_ExtentLoaderConfigs._InMemoryLoaderConfig.workspaceId, "Data");
