...
public static class ReportTests
{
    ...
    private static string GetScriptFolder([CallerFilePath] string path = "") =>
    ...
    public static async Task TestReports(bool doIssues , IElement configuration)
    {
        ...
        await using var dm = await GiveMe.DatenMeister(settings);
        ...
        await using var writer = ReportHelper.CreateRandomFile(out var fileName, targetPath);
