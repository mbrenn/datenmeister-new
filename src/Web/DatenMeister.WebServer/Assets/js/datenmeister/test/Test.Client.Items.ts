    ...
    describe('Client', function () {
        describe('Items', function () {
            ...
            it('Get Container', async () => {
                ...
                assert.isTrue(extentContainer[0].ententType === EntentType.Extent,
                    "Test 2: First item should be Extent");
                assert.isTrue(extentContainer[1].ententType === EntentType.Workspace, 
                ...
                assert.isTrue(extentContainer2[0].ententType === EntentType.Item,
                    "Test 5: First item should be Item");
                assert.isTrue(extentContainer2[0].uri === "dm:///unittest#" + result.itemId)
                assert.isTrue(extentContainer2[1].ententType === EntentType.Extent,
                    "Test 6: First item should be Extent");
                assert.isTrue(extentContainer2[2].ententType === EntentType.Workspace,
                ...
                assert.isTrue(extentContainer4[0].ententType === EntentType.Item,
                    "Test 17 First item should be Item");
                assert.isTrue(extentContainer4[0].uri === "dm:///unittest#" + subChild.itemId, 
                    "Test 18")
                assert.isTrue(extentContainer4[1].ententType === EntentType.Item,
                    "Test 19: First item should be Item");
                assert.isTrue(extentContainer4[1].uri === "dm:///unittest#" + result.itemId, 
                    "Test 20")
                assert.isTrue(extentContainer4[2].ententType === EntentType.Extent,
                    "Test 21: First item should be Extent");
                assert.isTrue(extentContainer4[3].ententType === EntentType.Workspace,
            ...
            it('Set Reference Property', async () => {
                ...
                assert.isTrue((reference as Mof.DmObject).get("name") === "item2", "Name is not correctly set");                
            });
            
            it('Add Items to Packages and Extent', async () => {
                // First, create a new item in the extent
                const result = 
                    await ClientItems.addToContainer("Test", "dm:///unittest",
                        {
                ...
                const result2 = 
                    await ClientItems.addToContainer("Test", packageItem.uri,
                        {
                ...
                const packageChildren = await ClientItems.getProperty(
                    packageItem.workspace, packageItem.uri, _CommonTypes._Default._Package.packagedElement);
                assert.isTrue(Array.isArray(packageChildren));
                assert.isTrue(packageChildren.some((child: Mof.DmObject) => child.uri === actionItem.uri), "Action item should be in package children");                
            });
            ...
            it ( 'Import Xmi', async () =>{
                ...
                const asDmObject = child[0] as Mof.DmObject;
