...
/**
 * Maps ObjectType enum values to their corresponding TypeScript types.
...
/**
 * Helper type that extracts the correct return type based on the ObjectType parameter.
 */
type DmObjectReturnType<T extends ObjectType> = DmObjectReturnTypeMap[T];
...
export class DmObject {
    ...
    private readonly values: Array<ObjectValue>;
    ...
    constructor(metaClassUri?: string | undefined, metaclassWorkspace?: string) {
        this.values = new Array<ObjectValue>();
     ...
     */
    appendToArray(key: string, value: any) {
...
/**
 * JSON representation of a MOF object for server communication.
 * This interface defines the structure used when serializing DmObjects to JSON.
 */
export interface JsonFromMofObject{
    /** Local identifier */
    id?: string;

    /** Property values (only included for non-reference objects) */
    v?: any;

    /**
     * URI of the reference (only for reference objects)
     */
    r?: string;

    /**
     * Workspace of the reference (only for reference objects)
     */
    w?: string;
...
export function createJsonFromObject(element: DmObject) {
    ...
    if (!element.isReference) {
        ...
        const internalValues = (element as DmObject)["values"];
        for (const internalizedKey in internalValues) {
            if (!internalValues.hasOwnProperty(internalizedKey)) continue;
 ...
 */
export function convertJsonObjectToObjects(
    element: any,
    extent: string | undefined = undefined,
 ...
 */
export function convertJsonObjectToDmObject(
    element: object | string | undefined,
    extent: string | undefined = undefined,
    workspace: string | undefined = undefined): DmObjectWithSync | undefined {
    ...
    if (elementValues !== undefined && elementValues !== null) {
        for (let key in elementValues) {
            if (Object.prototype.hasOwnProperty.call(elementValues, key)) {
                ...
                (result as DmObject)["values"][internalizedKey] = objectValue;
            }
        }
