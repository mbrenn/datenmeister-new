    ...
    async createDom(dmElement) {
        ...
        if (!this.isReadOnly) {
            ...
            nowBtn.on('click', () => {
                ...
                if (this._month)
                    this._month.val(now.getMonth() + 1);
                if (this._day)
                    this._day.val(now.getDate());
                if (this._hour)
                    this._hour.val(now.getHours());
                if (this._minute)
                    this._minute.val(now.getMinutes());
            ...
            if (this._year)
                this._year.on('change', changeHandler);
            if (this._month)
                this._month.on('change', changeHandler);
            if (this._day)
                this._day.on('change', changeHandler);
            if (this._hour)
                this._hour.on('change', changeHandler);
            if (this._minute)
                this._minute.on('change', changeHandler);
        }
        return container;
    }
